...
async function fetchSearchHistory() {
  ...
  try {
    const response = await $fetch<{ prompt: string; nodes: string[] }[]>(
      `http://192.168.135.118:8000/api/prompt/history`,
      {
        method: "GET",
        query: { graph },
      },
...
async function doSearch(query: string, nodes: string[] = []) {
  ...
  try {
    const route = useRoute();
    const graph = route.params.id as string;
    const response = await $fetch<{ response: string }>(`http://192.168.135.118:8000/api/prompt`, {
      method: "POST",
      headers: {
        'Content-Type': 'application/json',
      },
...
<div ref="container" :class="$style.base">
  ...
  <div :class="$style.header">
    ...
    <div :class="$style.headerCenter">
      ...
      />
      <div :class="$style.languageSelectWrapper">
        <div
          :class="[$style.languageSelect, { [$style.languageSelectOpen]: languageDropdownOpen }]"
          @click="languageDropdownOpen = !languageDropdownOpen"
        >
          <span :class="$style.languageSelectText">
            {{ selectedLanguageLabel }}
          </span>
          <span :class="$style.languageSelectArrow">▼</span>
        </div>
        <div v-if="languageDropdownOpen" :class="$style.languageDropdown">
          <div
            v-for="lang in availableLanguages"
            :key="lang.value"
            :class="[$style.languageOption, { [$style.languageOptionSelected]: language === lang.value }]"
...
.languageSelectOpen .languageSelectArrow {
