...
def run_bayan(graph: nx.Graph, args: dict[str, Any]) -> dict[int, dict[str, list[str]]]:
    """Run the Bayan algorithm and return communities by level."""
    max_cluster_size = args.get("max_cluster_size", 10)
    use_lcc = args.get("use_lcc", True)
    if use_lcc:
        graph = stable_largest_connected_component(graph)

    modularity, coverage, communities, runtime, penalty = bayanpy.bayan(graph)

    results = {0: {i: cluster for i, cluster in enumerate(communities)}}

    cluster_data = {
        level: {
            str(cluster): [str(node) for node in nodes]
            for cluster, nodes in partition_data.items()
        }
        for level, partition_data in results.items()
    }

    print("graph:", graph)
    print("comms:", cluster_data)
